...
| 8.5–14 s | "Take a married couple in Ohio," kids 4 and 8, one earner: the change in net income drawn across earnings from $0 to $120,000. "$0 below $42,200 in earnings. +$1,600 from $58,000." "The refundable part stays at $1,700 per child, so the extra $1,600 only offsets income tax the current credit leaves unpaid." Source tag: "policyengine.py 6.1.1 · static" | `data/earnings_sweep.json` (policyengine_us axes, one point per $250; cross-checked with policyengine.py at 7 earnings levels) |
| 14–20 s | Zoom out to all 50 states and DC: 12,000 weighted draws of households, each placed at random in its assigned congressional district; gainers light up grouped by gain (about $800, $1,600, then $2,400 and more); federal cost, share gaining, children out of poverty. Source tag: "policyengine.py 6.1.1 · populace_us_2024 · static / poverty: Supplemental Poverty Measure". The 12,000 draws cover 6,976 distinct households | `data/national.json`, `data/households_sample.json`, district shapes from `data/district_geography.json` |
| 20–25 s | The same dots settle on the baseline of each income decile; bars show average change per household. Source tag: "policyengine.py 6.1.1 · populace_us_2024 · static" | `data/national.json` → `deciles.by_decile` (full sample, not the 12,000 draws) |
| 25–30 s | The dots form the wordmark; tagline; "Free and open source · policyengine.org" | — |

Each chart carries its own small print in its bottom-right corner (`data/video.json → sources`, built from the versions and dataset each run recorded), so the close stays clean.
